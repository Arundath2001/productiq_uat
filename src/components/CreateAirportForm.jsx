...
const CreateAirportForm = ({ onCreateAirport, onUpdateAirport, setShowCreateAirport, isCreating, isUpdating, initialData }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (initialData) {
                await onUpdateAirport(initialData._id, formData);
            } else {
                await onCreateAirport(formData);
            }
            setShowCreateAirport(false);
        } catch (err) {
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center bg-[#B9B9B969] bg-opacity-50 z-50">
            <div className="bg-white rounded-xl shadow-lg w-full max-w-md p-6 relative">
                <button
                    onClick={() => setShowCreateAirport(false)}
                ...
                <div className="mb-6">
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                        {initialData ? "Update the details of the airport" : "Add a new airport"}
                    </p>
                </div>
